-- ============================================================
-- Schéma initial - Application de gestion d'examens en ligne (QCM)
-- ============================================================
...
-- Garanti ICI au niveau base (contrainte UNIQUE) ET revérifié côté
-- serveur avant toute écriture (voir exam.service.ts).
...
-- ------------------------------------------------------------
-- RG-09 (filet de sécurité en base, message propre géré côté service) :
--   - un cours possédant des examens ne peut pas être supprimé
--     -> exams.course_id est en ON DELETE RESTRICT
--   - un examen possédant des tentatives ne peut pas être supprimé
--     -> attempts.exam_id est en ON DELETE RESTRICT
-- Le service vérifie ces conditions AVANT le DELETE pour renvoyer
-- une erreur 409 propre au lieu de laisser remonter l'erreur SQL brute.
-- ------------------------------------------------------------
